...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      map = new google.maps.Map(container, {
        center: {
          lat: mapPickerCoords.lat || 20,
          lng: mapPickerCoords.lng || 0,
        },
        zoom: 4,
      });
      // @ts-ignore
    ...
    if (!apiKey) {
      toast({
        ...
        description: "Set VITE_GOOGLE_MAPS_API_KEY to enable map picking",
        variant: "destructive",
      });
      return;
    }
  ...
  const addInfluencer = () => {
    const id = `i${Date.now()}`;
    const categories = (newInfluencer.categories || "")
      .split(",")
      .map((s: string) => s.trim())
      .filter(Boolean);
    ...
    setInfluencers([
      ...influencers,
      {
        ...
        place: newInfluencer.place,
        latitude: newInfluencer.latitude
          ? Number(newInfluencer.latitude)
          : undefined,
        longitude: newInfluencer.longitude
          ? Number(newInfluencer.longitude)
          : undefined,
        categories,
        profiles,
      },
  ...
  const createJob = () => {
    const id = `j${Date.now()}`;
    const duration = {
      value: Number(newJob.durationValue || 0),
      unit: newJob.durationUnit || "days",
    };
    setJobs([
      ...jobs,
      {
        ...
        postUrl: newJob.postUrl,
        business: newJob.business,
        status: newJob.status || "Draft",
        budget: Number(newJob.budget || 0),
        bidStart: newJob.bidStart || undefined,
        bidEnd: newJob.bidEnd || undefined,
        duration,
      },
    ]);
    ...
    setBusinesses([
      ...businesses,
      {
        ...
        latitude: newBusiness.latitude
          ? Number(newBusiness.latitude)
          : undefined,
        longitude: newBusiness.longitude
          ? Number(newBusiness.longitude)
          : undefined,
      },
    ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="flex min-h-screen">
        {/* Sidebar */}
        <aside
          className={`${collapsed ? "w-20" : "w-64"} bg-white border-r border-gray-200 hidden md:flex flex-col h-screen transition-all duration-200`}
        >
          ...
          <nav className="flex-1 p-4 space-y-2 text-sm">
            <button
              ...
              className={`w-full flex items-center ${collapsed ? "justify-center px-2" : "text-left px-3 py-2"} rounded ${section === "dashboard" ? "bg-gray-100 font-medium" : "hover:bg-gray-50"}`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center ${collapsed ? "justify-center px-2" : "text-left px-3 py-2"} rounded ${section === "businesses" ? "bg-gray-100 font-medium" : "hover:bg-gray-50"}`}
            >
              <Store className="inline-block" />
              {!collapsed && <span className="ml-2">Businesses</span>}
            </button>
            <button
              onClick={() => setSection("influencers")}
              className={`w-full flex items-center ${collapsed ? "justify-center px-2" : "text-left px-3 py-2"} rounded ${section === "influencers" ? "bg-gray-100 font-medium" : "hover:bg-gray-50"}`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center ${collapsed ? "justify-center px-2" : "text-left px-3 py-2"} rounded ${section === "jobs" ? "bg-gray-100 font-medium" : "hover:bg-gray-50"}`}
            >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full">
                    <Input
                      ...
                      className="h-9 w-full sm:w-auto flex-1"
                    />
                    <Button
                      onClick={() => setShowAddManager(true)}
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              size="sm"
                              variant={
                                m.active ? undefined : ("outline" as any)
                              }
                              onClick={() => toggleManagerActive(m.id)}
                            >
            ...
            {section === "jobs" && (
              <div className="space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full">
                    <Input
                      placeholder="Filter by business, status..."
                      className="h-9 w-full sm:w-auto flex-1"
                    />
                    <Button
                        ...
                        <div className="space-y-3">
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <div>
                              ...
                              <div className="text-xs text-gray-500">
                                Choose light or dark — changes apply immediately
                              </div>
                            </div>
                            <div>
                              <select
          ...
          <div className="bg-white max-w-2xl w-full rounded shadow p-6 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2 flex items-center gap-3">
                  <Button
                    variant="outline"
                    onClick={() => {
                      setMapPickerTarget("influencer");
                      setMapPickerCoords({
                        lat: newInfluencer.latitude
                          ? Number(newInfluencer.latitude)
                          : undefined,
                        lng: newInfluencer.longitude
                          ? Number(newInfluencer.longitude)
                          : undefined,
                      });
                  ...
                  <div className="text-sm text-gray-500">
                    Selected:{" "}
                    {newInfluencer.latitude && newInfluencer.longitude
                      ? `${Number(newInfluencer.latitude).toFixed(6)}, ${Number(newInfluencer.longitude).toFixed(6)}`
                      : "None"}
                  </div>
                </div>
              </div>
                      ...
                      <div className="grid grid-cols-12 gap-2">
                        ...
                        <div className="col-span-12 sm:col-span-4">
                          <select
                            ...
                            value={p.platform}
                            onChange={(e) => {
                              const profiles = [...newInfluencer.profiles];
                              profiles[idx] = {
                                ...profiles[idx],
                                platform: e.target.value,
                              };
                              setNewInfluencer({ ...newInfluencer, profiles });
                            }}
                          >
              ...
              <div>
                ...
                <Input
                  value={newInfluencer.categories}
                  onChange={(e) =>
                    setNewInfluencer({
                      ...newInfluencer,
          ...
          <div className="bg-white max-w-2xl w-full rounded shadow p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="text-sm text-gray-500">
                  {selectedBusiness.address} • {selectedBusiness.industry}
                </div>
              </div>
              <div>
                <Button
            ...
            <div>
              ...
              <div className="space-y-2">
                {jobs
                  .filter((j) => j.business === selectedBusiness.name)
                  .map((j) => (
                    <div
                      key={j.id}
                      className="flex items-center justify-between p-2 border rounded"
                ...
                {jobs.filter((j) => j.business === selectedBusiness.name)
                  .length === 0 && (
                  <div className="text-sm text-gray-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2 flex items-center gap-3">
                  <Button
                    variant="outline"
                    onClick={() => {
                      setMapPickerTarget("business");
                      setMapPickerCoords({
                        lat: newBusiness.latitude
                          ? Number(newBusiness.latitude)
                          : undefined,
                        lng: newBusiness.longitude
                          ? Number(newBusiness.longitude)
                          : undefined,
                      });
                      setShowMapPicker(true);
                  ...
                  <div className="text-sm text-gray-500">
                    ...
                    {newBusiness.latitude && newBusiness.longitude
                      ? `${Number(newBusiness.latitude).toFixed(6)}, ${Number(newBusiness.longitude).toFixed(6)}`
                      : "None"}
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white max-w-3xl w-full rounded shadow p-6 max-h-[85vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600">
                Selected:{" "}
                {mapPickerCoords.lat
                  ? `${mapPickerCoords.lat.toFixed(6)}, ${mapPickerCoords.lng?.toFixed(6)}`
                  : "None"}
              </div>
              <div>
                <Button
                  ...
                  onClick={() => {
                    if (mapPickerCoords.lat && mapPickerCoords.lng) {
                      if (mapPickerTarget === "business") {
                        setNewBusiness({
                          ...newBusiness,
                          latitude: String(mapPickerCoords.lat),
                          longitude: String(mapPickerCoords.lng),
                        });
            ...
            <div className="mt-4 text-xs text-gray-500">
              This map uses the Google Maps JavaScript API. Set
              VITE_GOOGLE_MAPS_API_KEY in your environment for the map to load.
            </div>
          </div>
        </div>
      )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewJob({ ...newJob, postUrl: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <select
                    value={newJob.business}
                    onChange={(e) =>
                      setNewJob({ ...newJob, business: e.target.value })
                    }
                    className="w-full border rounded h-9 px-2"
                  >
                ...
                <div>
                  ...
                  <Input
                    type="datetime-local"
                    value={newJob.bidStart}
                    onChange={(e) =>
                      setNewJob({ ...newJob, bidStart: e.target.value })
                    }
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    value={newJob.bidEnd}
                    onChange={(e) =>
                      setNewJob({ ...newJob, bidEnd: e.target.value })
                    }
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input
                      type="number"
                      value={newJob.durationValue}
                      onChange={(e) =>
                        setNewJob({ ...newJob, durationValue: e.target.value })
                      }
                    />
                    <select
                      value={newJob.durationUnit}
                      onChange={(e) =>
                        setNewJob({ ...newJob, durationUnit: e.target.value })
                      }
          ...
          <div className="bg-white max-w-3xl w-full rounded shadow p-6 max-h-[85vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="text-sm text-gray-600">
                  {selectedInfluencer.email} • {selectedInfluencer.phone}
                </div>
                <div className="text-sm text-gray-500">
                  {selectedInfluencer.place}
                  {selectedInfluencer.latitude
                    ? ` • ${selectedInfluencer.latitude}, ${selectedInfluencer.longitude}`
                    : ""}
                </div>
              </div>
              <div>
                ...
                {selectedInfluencer.profiles.map((p) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {p.url} • {p.followers?.toLocaleString()} followers
                      </div>
                    </div>
                    <div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {jobs
                  .filter((j) =>
                    (j.participants || []).includes(selectedInfluencer.id),
                  )
                  .map((j) => (
                    <div
                      key={j.id}
                ...
                {jobs.filter((j) =>
                  ...
                  <div className="text-sm text-gray-500">
                    No campaign participation records found.
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white max-w-2xl w-full rounded shadow p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold">
                  {selectedJob.type} - {selectedJob.platform}
                </h3>
                <div className="text-sm text-gray-500">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <div className="text-sm">
                  {selectedJob.bidStart || "N/A"} to{" "}
                  {selectedJob.bidEnd || "N/A"}
                </div>
              </div>
              {selectedJob.duration && (
                <div>
                  ...
                  <div className="text-sm">
                    {selectedJob.duration.value} {selectedJob.duration.unit}
                  </div>
                </div>
              )}
            </div>
